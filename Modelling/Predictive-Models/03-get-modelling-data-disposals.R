...
disposals_all <- disposals_all |>
  ...
  mutate(
    disposals_lag_1 = lag(disposals, 1),
    disposals_lag_2 = lag(disposals, 2),
    disposals_lag_3 = lag(disposals, 3),
    disposals_lag_4 = lag(disposals, 4),
    disposals_lag_5 = lag(disposals, 5),
    disposals_lag_6 = lag(disposals, 6),
    disposals_lag_7 = lag(disposals, 7),
    disposals_lag_8 = lag(disposals, 8),
    disposals_lag_9 = lag(disposals, 9),
    disposals_lag_10 = lag(disposals, 10)
  ) |>
  ungroup()

# Get max round and season
max_round_data <- disposals_all |> arrange(desc(start_time_utc)) |> slice_head(n = 1)
...
get_last_n_avgs <- function(input_round, input_season) {
  ...
  last_3_stats <- player_data |> 
    ...
    slice_head(n = 3) |>  
    summarise(
      last_3_avg = mean(disposals),
      last_3_min = min(disposals),
      last_3_max = max(disposals),
      last_3_variance = var(disposals)
    )
  ...
  last_5_stats <- player_data |> 
    ...
    slice_head(n = 5) |>  
    summarise(
      last_5_avg = mean(disposals),
      last_5_min = min(disposals),
      last_5_max = max(disposals),
      last_5_variance = var(disposals)
    )
  ...
  last_7_stats <- player_data |> 
    ...
    slice_head(n = 7) |>  
    summarise(
      last_7_avg = mean(disposals),
      last_7_min = min(disposals),
      last_7_max = max(disposals),
      last_7_variance = var(disposals)
    )
  ...
  last_10_stats <- player_data |> 
    ...
    slice_head(n = 10) |>  
    summarise(
      last_10_avg = mean(disposals),
      last_10_min = min(disposals),
      last_10_max = max(disposals),
      last_10_variance = var(disposals)
    )
...
training <-
  ...
  select(player_full_name, player_team, opposition_team, margin, round, season = season_name, contains("disposals"), home_away) |> 
...
training |> write_csv("Modelling/Predictive-Models/Data/training_data_disposals.csv")
test |> write_csv("Modelling/Predictive-Models/Data/test_data_disposals.csv")
target |> write_csv("Modelling/Predictive-Models/Data/target_data_disposals.csv")

training |> write_rds("Modelling/Predictive-Models/Data/training_data_disposals.rds")
test |> write_rds("Modelling/Predictive-Models/Data/test_data_disposals.rds")
target |> write_rds("Modelling/Predictive-Models/Data/target_data_disposals.rds")
